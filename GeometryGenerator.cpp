...
void GeometryGenerator::CreateSphere(float radius, UINT sliceCount, UINT stackCount, MeshData& meshData)
{
	...
	for(UINT i = 1; i <= stackCount-1; ++i)
	{
		...
		for(UINT j = 0; j <= sliceCount; ++j)
		{
			...
			D3DXVECTOR3 T;
			D3DXVec3Normalize(&T, &v.TangentU);
			v.TangentU = T;
...
void GeometryGenerator::CreateGeosphere(float radius, UINT numSubdivisions, MeshData& meshData)
{
	...
	for(UINT i = 0; i < meshData.Vertices.size(); ++i)
	{
		...
		meshData.Vertices[i].TexC.x = theta/D3DX_PI;
		meshData.Vertices[i].TexC.y = phi/D3DX_PI;
		...
		D3DXVECTOR3 T;
		D3DXVec3Normalize(&T, &meshData.Vertices[i].TangentU);
...
void GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, UINT sliceCount, UINT stackCount, MeshData& meshData)
{
	...
	for(UINT i = 0; i < ringCount; ++i)
	{
		...
		for(UINT j = 0; j <= sliceCount; ++j)
		{
			...
			D3DXVec3Cross(&vertex.Normal, &T, &B);
...
//2 x- 3 x+
//4 y- 5 y+
static const D3DXVECTOR3 boxNormal[6] = {
	D3DXVECTOR3( 0, 0,-1),
	D3DXVECTOR3( 0, 0, 1),
	D3DXVECTOR3(-1, 0, 0),
	D3DXVECTOR3( 1, 0, 0),
...
void GeometryGenerator::CreateLightVolume(std::vector<D3DXVECTOR3> &vs, 
	std::vector<UINT16> &is,
	D3DXVECTOR3 boxVert[],
	D3DXVECTOR3 lightDir)
{
	static const float tmin = 1000000.0f;
	D3DXVECTOR3 dir = lightDir;
	int idcnt = 0;
	int idv[16];
	...
	for (int i = 0; i < 6; i++)
	{
		passp[i] = D3DXVec3Dot(boxNormal + i, &dir) < 0;
		if (passp[i])
		{
			for (int j = 0; j < 4; j++)
			{
		...
		if (passp[i])
		{
			...
			for (int j = 0; j < 4; j++)
			{
				if (passe[pt[j]][pt[j + 1 & 3]] == 1)
				{
					...
					{
						idv[v + 8] = vs.size();
						vs.push_back(boxVert[v] + dir * tmin);
					}

					if (idv[u + 8] < 0)
					{
						idv[u + 8] = vs.size();
						vs.push_back(boxVert[u] + dir * tmin);
					}
